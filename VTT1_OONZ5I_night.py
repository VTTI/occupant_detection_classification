    ...
    for f in files:
        if f.endswith('.txt'):
            for p in loaded_data['images']:
                if os.path.splitext(f)[0]==p['file_name'] and f[-9:-4]=='Night':
                    front_found= False
                    image_id= p['id']
...
# pyplot.savefig('ROC_front_night.png')
# pyplot.close()
...
# # false_positives= []
# # for i,j in enumerate(yconf):
# #     if j==0 and back_ground_truth[i]==1:
...
# #                         if bbox_area > 30000 and x < horizontal_limit and y > vertical_limit:
# #                             print('found')
# #             cnt+= 1


# # #ROC curve
# # fpr, tpr, thresholds = metrics.roc_curve(driver_ground_truth, driver_pred_res, pos_label=1)
# # pyplot.plot(fpr, tpr, marker='.', label='Driver')
...
# # pyplot.close()
# # calculate AUC
# # roc_auc = roc_auc_score(ground_truth, pred_res)
# # print('ROC curve AUC: %.3f' % roc_auc)

# ##Precision recall curve
# # precision, recall, thresholds = precision_recall_curve(ground_truth, pred_res)
# # # calculate precision-recall AUC
# # pr_auc = auc(recall, precision)
# # print('PR curve AUC: %.3f' % (pr_auc))
...
# #         else:
# #             yhat.append(1)
# #     pr_f1 = f1_score(ground_truth, yhat)
# #     if pr_f1>max:
# #         max= pr_f1
# #         index= limit
# # print('PR curve max F1: %.3f at confidence threshold %.3f' %(max,index))

# # pyplot.plot(recall, precision, marker='.', label='Faster RCNN 101')
